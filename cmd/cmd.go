...
func (h helpCmd) ShortDesc() string { return `prints help information for other commands.` }
...
func (h helpCmd) Exec(ctx context.Context, args []string) (context.Context, error) {
	...
	cmdsVal := ctx.Value("shell.commands")
	if cmdsVal == nil {
		return ctx, errors.New("nil context")
	}
...
func (c exitCmd) ShortDesc() string { return `exits the interactive shell immediately` }
...
func (c promptCmd) ShortDesc() string { return `sets a new shell prompt` }
func (c promptCmd) Exec(ctx context.Context, args []string) (context.Context, error) {
	...
	return context.WithValue(ctx, "shell.prompt", args[1]), nil
}
...
func (c sysinfoCmd) ShortDesc() string { return `sets a new shell prompt` }
...
func (c *sysCommands) Registry() map[string]api.Command {
